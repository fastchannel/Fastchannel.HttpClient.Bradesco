...
namespace Fastchannel.HttpClient.Bradesco.RestClient
{
    internal class RestClient : IDisposable
    {
        ...
        private System.Net.Http.HttpClient HttpClient => _httpClient ?? (_httpClient = BuildHttpClient());
        ...
        private System.Net.Http.HttpClient BuildHttpClient()
        {
            var webHandler = new HttpClientHandler { UseCookies = false };
            _httpClient = new System.Net.Http.HttpClient(webHandler)
            {
        ...
        private CancellationToken CreateRequestTimeout(Request request)
        {
            ...
            var defaultTimeout = _defaultTimeoutInSeconds ?? default;
            var requestTimeout = request?.TimeoutInSeconds ?? default;
